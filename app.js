...
tg.ready(); tg.expand(); tg.setHeaderColor("#0a0a12");

const API_URL = "https://sex-512.onrender.com";     // ← ТВОЙ URL
const BOT_USERNAME = "free_stars_infbot";            // ← БЕЗ @
...
async function loadMe() {
    try {
        ...
        if (d.is_admin) document.getElementById("adminTabBtn").classList.remove("hidden");
...
function shareRef() {
    ...
    const text = encodeURIComponent("🎰 Заходи в казино, играй и выигрывай!");
...
function initCanvas() {
    ...
    ctx.fillStyle = "#8b8ba7"; ctx.font = "16px sans-serif"; ctx.textAlign = "center";
...
function drawWheel(players, rotation, winnerIndex, highlight) {
    ...
    players.forEach((p, i) => {
        ...
        if (highlight && i === winnerIndex) { ctx.strokeStyle = "#fff"; ctx.lineWidth = 5; ctx.stroke(); }
    ...
    ctx.lineTo(cx + 12, cy - radius + 10);
    ctx.closePath(); ctx.fillStyle = "#ffcb52"; ctx.fill();
...
function drawSquare(players, dotPos, winnerIndex) {
    ...
    players.forEach((p, i) => {
        ...
        ctx.fillStyle = p.color; ctx.fillRect(cx, cy, cellW - 4, cellH - 4);
        ctx.strokeStyle = "#0a0a12"; ctx.lineWidth = 2; ctx.strokeRect(cx, cy, cellW - 4, cellH - 4);
