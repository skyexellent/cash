    ...
    async with pool.acquire() as conn:
        ...
        await conn.execute("""
            CREATE TABLE IF NOT EXISTS transactions (
                ...
                user_id BIGINT, amount BIGINT, type TEXT,
                description TEXT, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
            )
        """)
        await conn.execute("""
            CREATE TABLE IF NOT EXISTS rounds (
                id SERIAL PRIMARY KEY, game TEXT, pool BIGINT,
                winner_id BIGINT, winner_name TEXT,
        ...
        await conn.execute("""
            CREATE TABLE IF NOT EXISTS bets (
                id SERIAL PRIMARY KEY, round_id INTEGER,
                user_id BIGINT, username TEXT, stake BIGINT,
                created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
            )
        """)
        await conn.execute("""
            CREATE TABLE IF NOT EXISTS withdrawals (
                ...
                requisites TEXT, status TEXT DEFAULT 'pending',
                created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
            )
        """)
    log.info("✅ БД готова")
...
async def upsert_user(user_id: int, username: str, first_name: str, referrer_id: int | None = None):
    async with pool.acquire() as conn:
        ...
        if existing:
            ...
            await conn.execute(
                "INSERT INTO users (user_id, username, first_name, referrer_id) VALUES ($1,$2,$3,$4)",
...
async def get_top_players(limit: int = 10):
    async with pool.acquire() as conn:
        rows = await conn.fetch(
            "SELECT user_id, username, first_name, total_won, balance FROM users ORDER BY total_won DESC LIMIT $1",
            limit,
        )
        return [dict(r) for r in rows]
...
async def try_daily_bonus(user_id: int):
    async with pool.acquire() as conn:
        ...
        if last and datetime.utcnow() - last < timedelta(hours=24):
            ...
            return {"ok": False, "msg": f"Бонус через {h}ч {m}м"}
        await conn.execute(
        ...
        "players": [
            {
                ...
                "first_name": p["first_name"], "stake": p["stake"],
...
async def _lobby_countdown(game: str):
    ...
    try:
        ...
        if len(lob["players"]) >= 2:
            ...
            backup = list(lob["players"])
            lob["players"] = []
            await broadcast(game, "refunded", {"players": backup})
            await broadcast_state(game)
...
async def run_round(game: str):
    ...
    await broadcast(game, "spin", {
        ...
        "total": total, "payout": payout, "duration": SPIN_TIME,
    })
    ...
    await broadcast(game, "result", {
        "winner_id": winner["user_id"],
        "winner_name": winner["username"] or winner["first_name"],
        "payout": payout, "total": total, "commission": commission,
    })
...
async def api_me(request: Request):
    ...
    return {
        ...
        "first_name": user.get("first_name", ""), "balance": db_user["balance"],
        "total_topup": db_user["total_topup"], "total_spent": db_user["total_spent"],
        "total_won": db_user["total_won"], "ref_count": ref["count"],
        "ref_earned": ref["earned"], "is_admin": user["id"] in ADMIN_IDS,
    }
...
async def api_topup(request: Request):
    ...
    await bot.send_invoice(
        chat_id=user["id"], title=f"Пополнение на {amount}⭐",
        description=f"Зачислит {amount} звёзд",
        payload=f"topup:{amount}", provider_token="", currency="XTR",
...
async def api_admin_stats(request: Request):
    ...
    return {"users": users_count, "total_balance": total_balance,
...
async def api_game_bet(game: str, request: Request):
    ...
    colors = ["#ffcb52", "#38bdf8", "#10b981", "#ef4444", "#a855f7",
              "#ff8a3d", "#14b8a6", "#8b5cf6", "#84cc16", "#ec4899"]
    color = colors[len(lob["players"]) % len(colors)]
    lob["players"].append({
        "user_id": user["id"], "username": user.get("username", ""),
        "first_name": user.get("first_name", ""),
        "stake": amount, "color": color,
    })
    async with pool.acquire() as conn:
        await conn.execute(
            "INSERT INTO transactions (user_id, amount, type, description) VALUES ($1,$2,$3,$4)",
